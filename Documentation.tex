...
For instance, the dead key \key|G-S-[| adds a hook to the following letter,
  so typing something like \key|G-S-[ b| will generate the letter \out{ɓ}.
...
The allowed inputs for each dead key are specified in tabular format when each key is described.
\footnotetext{%
...
This term refers to sequences of two keys which only produce one letter when typed.
For instance, the left quote \out{‘} is produced by the key sequence \key|G-' S-(|, while the letter \out{ƣ} is produced by the key sequence \key|G-f g|.
...
To capitalise a letter which is produced by a key sequence, the \textsc{shift} key should only be pressed for the last key in the sequence.
For instance, the lowercase letter \out{ƣ} is produced by the key sequence \key|G-f g|,
  while the capital letter \out{Ƣ} is produced by the key sequence \key|G-f S-g|,
  rather than \key|G-S-f g| or \key|G-S-f S-g|.
...
\item \key|x| (in a monospace font) refers to the key(s) denoted by \key|x| (see Section~\ref{sec:key_notation} below for details).
\item \out{x} refers to the letter (or symbol) `x', as produced by Conkey.
\end{itemize}

The various letters and symbols of Conkey are listed within tables.
These tables are formatted using a consistent set of conventions.
For instance, here are some selected example rows:
...
For instance, pressing \key|G-d S-i| results in uppercase \out{Ɩ},
  but pressing \key|G-d S-k| does not give any sensible output.
(Recall that uppercase letters are typed by holding the \textsc{shift} key;
  for a key sequence, \textsc{shift} must only be held during the last key pressed.)
The `Uppercase?' column may be missing for tables containing symbols (which do not have uppercases).
...
Input & Result \\
\midrule
\key|G-] S-\| & Vertical line above ◌̍ \\
\key|G-] \|   & Vertical line below ◌̩ \\
\key|G-] S-'| & Double vertical line above ◌̎ \\
\key|G-] ,|   & Comma above ◌̓ \\
\key|G-] S-,| & Turned comma above ◌̒ \\
\key|G-] .|   & Reversed comma above  ◌̔ \\
\key|G-] S-.| & Comma above right ◌̕ \\
\key|G-] S-8| & Dot above right ◌͘ \\
\key|G-] `|   & Grave below ◌̖ \\
\key|G-] S-`| & Acute below ◌̗ \\
\midrule
\key|G-] '| & Modifier apostrophe ◌ʼ \\
\key|G-] p| & Modifier prime ◌ʹ \\
\key|G-] S-p| & Modifier double prime ◌ʺ \\
...
\key|G-] S-]| & Modifier lower right corner ◌ꜚ \\
...
Click letters can be produced using the key sequences starting with \key|G-p|.
...
Superscript letters and symbols can be typed using key sequences of the form \key|G-w <char>|,
...
Similarly, subscripts may be typed using key sequences of the form \key|G-S-w <char>|,
  where \key|<char>| is one of the following:
\begin{itemize}[noitemsep]
\item Any number
...
Zhuang tone letters can be accessed via key sequences starting with \key|G-1|.
...
Quotation marks and guillemots can be typed using key sequences starting with
...
\key|G-SPC S-\| & Hair space                          & \uni{U+200a} \\
\key|G-SPC S-=| & Medium mathematical space           & \uni{U+205f} \\
\key|G-SPC S-1| & Narrow no-break space               & \uni{U+202f} \\
\key|G-SPC 0|   & Zero width space                    & \uni{U+200b} \\
\key|G-SPC S-9| & Zero width non-joiner               & \uni{U+200c} \\
\key|G-SPC S-0| & Zero width joiner                   & \uni{U+200d} \\
...
Aside from the punctuation marks already covered above,
  ...
  mostly via key sequences starting with \key|G-S-2|.
...
\key|G-= 0|     & ∅ & \key|G-= S-c| & ℂ \\
\key|G-= e|     & ∈ & \key|G-= S-n| & ℕ \\
\key|G-= S-e|   & ∉ & \key|G-= S-q| & ℚ \\
\key|G-= S-u|   & ∪ & \key|G-= S-r| & ℝ \\
\key|G-= S-i|   & ∩ & \key|G-= S-z| & ℤ \\
\bottomrule
\end{tabular}
\end{minipage}
\end{table}
...
\end{itemize}
The key sequences for this dead key are shown in Table~\ref{tab:math}.
