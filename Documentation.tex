...
For instance, \key|G-_ r| means `press AltGr+shift+\key|-| at the same time, then release it, then press \key|r|'.
...
For instance, the lowercase letter \out{ƣ} is produced by the key sequence \key|G-f g|,
  ...
  rather than \key|G-F g| or \key|G-F S-G|.
...
For instance, although there is no precomposed Unicode character for \out{a̋},
  this may still be typed by first typing \key|a|, and then typing \key|G-: SPC| to produce the combining double acute accent.
...
Diaeresis/umlaut ◌̈ & \key|G-"|   & \ins{aehiotuwxy}{AEHIOUWXY}                     & Note that this dead key is also used for double quotes and guillemots. \\
...
Tilde below ◌̰      & \key|G-#|   & \ins*{elu}{elU}                                 & \\
...
% For instance, \key|G-l g| returns \out{ƣ}, and \key|G-l "| returns \out{Ꜣ}.
...
\begin{itemize}[noitemsep]
\item \out{ʘ} can be produced via \key|G-p @|
\item \out{ǀ} can be produced via \key!G-p |!, \key|G-p \| or \key|G-p c|
\item \out{ǃ} can be produced via \key|G-p !| or \key|G-p q|
...
Similarly, subscripts may be typed using key sequences of the form \key|G-W <char>|,
  where \key|<char>| is one of the following:
\begin{itemize}[noitemsep]
\item Any number
...
Although, strictly speaking, the chevrons \out{⟨} and \out{⟩} are brackets rather than quotes or guillemots,
  Conkey allows these to be typed as well using \key|G-< G-<| and \key|G-> G->| respectively.
